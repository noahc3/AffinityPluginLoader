...
/* For Unix builds, qsort() expects standard C calling convention, not __cdecl (ms_abi) */
#ifdef __unix__
#undef __cdecl
#define __cdecl
#endif
...
/* Calculate the maximum deviation of a cubic Bezier curve from its quadratic approximation.
 * This is used to determine if subdivision is needed for accurate representation. */
static float d2d_bezier_cubic_to_quadratic_error(const D2D1_POINT_2F *p0,
...
/* Subdivide a cubic Bezier curve at t=0.5 using De Casteljau's algorithm. */
static void d2d_bezier_cubic_subdivide(const D2D1_POINT_2F *p0, const D2D1_POINT_2F *p1,
        ...
        D2D1_BEZIER_SEGMENT *left, D2D1_BEZIER_SEGMENT *right, D2D1_POINT_2F *mid)
{
    ...
    /* Final subdivision point */
    mid->x = (r0.x + r1.x) * 0.5f;
    mid->y = (r0.y + r1.y) * 0.5f;

    /* Left cubic segment: p0, q0, r0, mid */
    left->point1 = q0;
    left->point2 = r0;
    left->point3 = *mid;

    /* Right cubic segment: mid, r1, q2, p3 */
    right->point1 = r1;
    right->point2 = q2;
    right->point3 = *p3;
}

/* Check if a cubic Bezier is simple enough for single quadratic approximation.
 * Returns false if the curve needs subdivision (inflection, high curvature, or control point deviation). */
static bool d2d_bezier_cubic_is_simple(const D2D1_POINT_2F *p0, const D2D1_POINT_2F *p1,
        const D2D1_POINT_2F *p2, const D2D1_POINT_2F *p3)
{
    float ax, ay, bx, by, cx, cy;
    float cross1, cross2;
    float chord_len, deviation1, deviation2;
    float dx, dy;

    /* Check for inflection point using cross products of control polygon edges. */

    /* Control polygon vectors */
    ax = p1->x - p0->x;
    ay = p1->y - p0->y;
    bx = p2->x - p1->x;
    by = p2->y - p1->y;
    ...
    /* Cross products between consecutive edges */
    cross1 = ax * by - ay * bx;  /* (P1-P0) × (P2-P1) */
    cross2 = bx * cy - by * cx;  /* (P2-P1) × (P3-P2) */

    /* If cross products have opposite signs, there's an inflection point */
    if (cross1 * cross2 < -0.1f)
        return FALSE;  /* Inflection point detected */

    /* Check control point deviation from chord (P0 to P3).
     * If control points deviate too much, the curve is too complex. */
    dx = p3->x - p0->x;
    dy = p3->y - p0->y;
    chord_len = sqrtf(dx * dx + dy * dy);

    if (chord_len < 1.0f)
        return TRUE;  /* Very short curve - always simple */

    /* Distance of P1 from chord P0-P3 */
    deviation1 = fabsf((p1->x - p0->x) * dy - (p1->y - p0->y) * dx) / chord_len;
    /* Distance of P2 from chord P0-P3 */
    deviation2 = fabsf((p2->x - p0->x) * dy - (p2->y - p0->y) * dx) / chord_len;

    /* Adaptive threshold based on chord length - tighter for longer segments */
    float max_deviation_ratio = (chord_len > 50.0f) ? 0.15f : 0.20f;

    /* If control points deviate too much from chord, subdivide */
    if (deviation1 > chord_len * max_deviation_ratio || deviation2 > chord_len * max_deviation_ratio)
        return FALSE;  /* Control points too far from chord - high curvature */

    /* For very tight bends (high absolute deviation), always subdivide */
    if (deviation1 > 10.0f || deviation2 > 10.0f)
        return FALSE;  /* Very tight curve - needs subdivision */

    /* Check if control points pull in opposite directions (S-curve indicator) */
    if (deviation1 > 3.0f && deviation2 > 3.0f)
    {
        /* Calculate signed distances to detect opposite-side pulling */
        float signed1 = (p1->x - p0->x) * dy - (p1->y - p0->y) * dx;
        float signed2 = (p2->x - p0->x) * dy - (p2->y - p0->y) * dx;
        if (signed1 * signed2 < 0)
            return FALSE;  /* S-curve detected */
    }

    return TRUE;  /* Simple curve - can approximate */
}

/* Recursively convert a cubic Bezier to quadratic Bezier segments with adaptive subdivision. */
static bool d2d_figure_add_cubic_bezier_recursive(struct d2d_figure *figure,
        ...
        float tolerance, unsigned int depth)
{
    D2D1_POINT_2F q, mid;
    ...
    if (depth > 16)
    {
        WARN("Maximum recursion depth reached for cubic Bezier subdivision - using line segment.\n");
        /* At max depth, just use a line segment as fallback to ensure SOMETHING renders */
    ...
    /* Subdivide if: error too large OR curve has inflection point */
    if (error <= tolerance && is_simple)
    {
    ...
    /* Error too large - subdivide and recurse */
    d2d_bezier_cubic_subdivide(p0, &cubic->point1, &cubic->point2, &cubic->point3,
            &left, &right, &mid);

    if (!d2d_figure_add_cubic_bezier_recursive(figure, p0, &left, tolerance, depth + 1))
        return false;

    /* The midpoint is now the current vertex, continue with right segment */
    if (!d2d_figure_add_cubic_bezier_recursive(figure, &mid, &right, tolerance, depth + 1))
        return false;
        ...
        UINT32 count)
{
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        /* Convert cubic Bezier to quadratic segments with adaptive subdivision */
        if (!d2d_figure_add_cubic_bezier_recursive(figure, p0, &beziers[i], tolerance, 0))
            return false;
    }
